  ...
  if (step >= 4) {
    items.push(
      ...
      },
      {
        id: 'sla',
        kind: 'joined',
        text: '⏱ 미응답 알림 설정 — 1시간 내 미응답 시 SV·본사에 동시 알림',
        isNew: step === 4,
      },
    );
  }
...
const VALUE: MfgValueStripDef = {
  ...
  pain: '해결: 개인폰 귀속 → 본사 기록 · 카톡 미사용 점주 사각 → 문자·웹 참여 · 응대 누락 → 1시간 미응답 알림',
  ...
  states: [
    ...
    },
    {
      ...
      memo: {
        ...
        meta: '신 1 · ROI',
        situation:
          '신 0에서 묻히던 그 문의가 이번엔 본사 채널로 들어온다. 1시간 내 미응답이면 슈퍼바이저와 본사에 동시 알림이 가므로, 1시간 35분 무응답 같은 일이 구조적으로 걸러진다.',
